...
sealed class PasswordResetTokenExtractionException(
     ...
     * @param cause The underlying cause of the exception, if available.
     *
     * @property code The error code `INVALID_PASSWORD_RESET_TOKEN`
    ...
    class Invalid(msg: String, cause: Throwable? = null) : PasswordResetTokenExtractionException(
        ...
        "Indicates that the password reset token cannot be decoded.",
        cause
    )
     ...
     * @param cause The underlying cause of the exception, if available.
     *
     * @property code `PASSWORD_RESET_TOKEN_ALLOWLIST_READING_FAILURE`
