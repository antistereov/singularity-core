...
class OAuth2Service(
    ...
    private val logger = KotlinLogging.logger {}
     ...
     suspend fun findOrCreateUser(oauth2Token: OAuth2AuthenticationToken): UserDocument {
         ...
         return userService.findByIdentityOrNull(provider, principalId)
             ?: handleRegistration(name, email, provider, principalId)
    }
    ...
    private suspend fun handleRegistration(name: String, email: String, provider: String, principalId: String): UserDocument {
        ...
        if (userService.existsByEmail(email)) handleAccountLink()
    ...
    private suspend fun handleAccountLink() {
        logger.debug { "Handling account link" }
