...
class PrincipalMapper(
    ...
    /**
     * Converts a given user and authentication outcome into a UserResponse object encapsulated
     ...
     * @return A [Result] containing a [UserResponse] object when successful, or a [PrincipalMapperException] in case of failure.
     */
    ...
    /**
     * Converts a given user and authentication outcome into a UserOverviewResponse object
     ...
     * @return A [Result] containing a [UserOverviewResponse] when successfully mapped, or a [PrincipalMapperException]
     * in case of a mapping failure.
     */
    suspend fun toOverview(
        principal: Principal<out Role, out SensitivePrincipalData>,
        authenticationOutcome: AuthenticationOutcome
    ): Result<UserOverviewResponse, PrincipalMapperException> = coroutineBinding {
        logger.debug { "Creating UserOverviewResponse for user with ID \"${principal.id}\"" }
