...
class GeoLocationServiceTest : BaseIntegrationTest() {
    ...
    @Test fun `it should resolve geolocation in device`() = runTest {
        val email = "test@example.com"
        val deviceId = "123456"
        val body = RegisterUserRequest(email = email, password = "password", "Name", DeviceInfoRequest(id = deviceId))
        ...
        val location = user.sensitive.devices.firstOrNull { it.id == deviceId }?.location
