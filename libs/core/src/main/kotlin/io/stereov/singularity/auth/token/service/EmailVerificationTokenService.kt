...
/**
 * A service responsible for creating and extracting [EmailVerificationToken]s.
 *
...
class EmailVerificationTokenService(
     ...
     * or a [TokenCreationException] if an error occurs during token creation.
     */
    suspend fun create(
        userId: ObjectId,
        email: String,
     ...
     * or a [EmailVerificationTokenExtractionException] if an error occurs during extraction.
     */
    suspend fun extract(token: String): Result<EmailVerificationToken, EmailVerificationTokenExtractionException> {
        logger.debug { "Validating email verification token" }

        return jwtService.decodeJwt(token, tokenType)
            .mapError { ex -> EmailVerificationTokenExtractionException.fromTokenExtractionException(ex) }
            .andThen { jwt -> binding {
                val userId = jwt.subject
                    .toResultOr { EmailVerificationTokenExtractionException.Invalid("Email verification token does not contain sub claim") }
                    .andThen { sub ->
                        runCatching { ObjectId(sub) }.mapError { ex ->
                            EmailVerificationTokenExtractionException.Invalid("Invalid ObjectId in sub: $sub", ex)
                        }
                    }.bind()

                val email = (jwt.claims["email"] as? String)
                    .toResultOr { EmailVerificationTokenExtractionException.Invalid("No email found in claims") }
                    .bind()

                val secret = jwt.id
                    .toResultOr { EmailVerificationTokenExtractionException.Invalid("Email verification token does not contain ID") }
                    .bind()
