    ...
    */
    suspend fun sendVerificationEmail(user: UserDocument, newEmail: String? = null) {
        ...
        if (mailCooldownService.getRemainingVerificationCooldown(userId) > 0) {
            throw io.stereov.singularity.global.service.mail.exception.model.MailCooldownException(
                mailCooldownService.getRemainingVerificationCooldown(
                    userId
                )
            )
        }
    ...
    suspend fun sendPasswordResetEmail(user: UserDocument) {
        ...
        if (mailCooldownService.getRemainingPasswordResetCooldown(userId) > 0) {
            throw io.stereov.singularity.global.service.mail.exception.model.MailCooldownException(
                mailCooldownService.getRemainingPasswordResetCooldown(
                    userId
                )
            )
        }
